	...
	if initial_expression_properties != null:
		for key in initial_expression_properties.keys():
			if not key is String and not key is StringName:
				push_error(
					"Expression property names must be strings. Ignoring initial expression property with key ",
					key,
				)
				continue
...
func send_event(event: StringName) -> void:
	...
	if not is_node_ready():
		push_error(
			"State chart is not yet ready. If you call `send_event` in _ready, please call it deferred, e.g. `state_chart.send_event.call_deferred(\"my_event\")."
		)
		return
	...
	if (
		warn_on_sending_unknown_events and event != ""
		and OS.is_debug_build() and not _valid_event_names.has(event)
	):
		push_warning(
			"State chart does not have an event '",
			event,
			"' defined. Sending this event will do nothing.",
		)
...
func set_expression_property(property_name: StringName, value) -> void:
	...
	if not is_node_ready():
		push_error(
			"State chart is not yet ready. If you call `set_expression_property` in `_ready`, please call it deferred, e.g. `state_chart.set_expression_property.call_deferred(\"my_property\", 5)."
		)
		return
...
func _run_queued_transitions() -> void:
	...
	while _queued_transitions.size() > 0:
		...
		if execution_count > 100:
			push_error(
				"Infinite loop detected in transitions. Aborting. The state chart is now in an invalid state and no longer usable."
			)
			break
...
func _warn_not_active(transition: Transition, source: StateChartState) -> void:
	push_warning(
		"Ignoring request for transitioning from ",
		source.name,
		" to ",
		transition.to,
...
func step() -> void:
	...
	if not is_node_ready():
		push_error(
			"State chart is not yet ready. If you call `step` in `_ready`, please call it deferred, e.g. `state_chart.step.call_deferred()`."
		)
		return
